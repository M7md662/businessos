...
`;
    ...
    const normalizedMessage = Array.from(message.normalize("NFD")).filter((char) => { const code = char.codePointAt(0) || 0; return !((code >= 0x0300 && code <= 0x036f) || (code >= 0x064b && code <= 0x065f) || code === 0x0670); }).join("").replace(/[?]/g, "").trim();

    const createOrderRequested =
      /(?:create\\s+order|add\\s+order|new\\s+order)/i.test(
        normalizedMessage
      ) ||
      normalizedMessage.includes(
        String.fromCodePoint(
          0x0627,0x0646,0x0634,0x0626,0x0020,
          0x0637,0x0644,0x0628
        )
      ) ||
      normalizedMessage.includes(
        String.fromCodePoint(
          0x0627,0x0646,0x0634,0x064a,0x0020,
          0x0637,0x0644,0x0628
        )
      ) ||
      normalizedMessage.includes(
    ...
    const createTaskRequested =
      ...
      /(?:create\\s+task|add\\s+task|new\\s+task)/i.test(
        normalizedMessage
      ) ||
      (!createOrderRequested &&
        (
